...
try {
    ...
    if (isset($_GET['del'])) {
        ...
        header("Location: pizzak"); // Frissítés, hogy eltűnjön az URL-ből
    }

    if (isset($_POST['mentes'])) {
        if ($_POST['id'] > 0) {
            // Módosítás (Update)
            $sql = "UPDATE pizzak SET nev = :nev, feltet = :feltet, ar = :ar WHERE id = :id";
            $sth = $dbh->prepare($sql);
            $sth->execute(array(
                ':nev' => $_POST['nev'],
                ':feltet' => $_POST['feltet'],
            ...
            // Új felvitele (Create)
            $sql = "INSERT INTO pizzak (nev, feltet, ar) VALUES (:nev, :feltet, :ar)";
            $sth = $dbh->prepare($sql);
            $sth->execute(array(
                ':nev' => $_POST['nev'],
                ':feltet' => $_POST['feltet'],
    ...
    $edit_pizza = array('id' => 0, 'nev' => '', 'feltet' => '', 'ar' => '');
    if (isset($_GET['edit'])) {
...
<div style="background: #f8edeb; padding: 20px; border-radius: 8px; margin-bottom: 30px;">
    ...
    <form action="pizzak" method="post">
        ...
        <input type="text" name="nev" value="<?= htmlspecialchars($edit_pizza['nev']) ?>" required><br>
        
        <label>Feltétek:</label><br>
        <input type="text" name="feltet" value="<?= htmlspecialchars($edit_pizza['feltet']) ?>" required><br>
        
        <label>Ár (Ft):</label><br>
        <input type="number" name="ar" value="<?= $edit_pizza['ar'] ?>" required><br><br>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <a href="pizzak&edit=<?= $p['id'] ?>" style="color: blue; font-weight: bold; margin-right: 15px;">Szerkesztés</a>
                <a href="pizzak&del=<?= $p['id'] ?>" style="color: red; font-weight: bold;" onclick="return confirm('Biztosan törlöd?')">Törlés</a>
            </td>
        </tr>
        <?php endforeach; ?>
    </tbody>
